...
def admin_index(request):
    ...
    total_earnings = Pedido.objects.filter(pagado=True).aggregate(
        total=Sum('total')
    ...
    total_customers = Usuario.objects.filter(rol='cliente').count()
    
    # Balance (ingresos - puede incluir lógica de costos si se tiene)
    my_balance = total_earnings  # Por ahora es igual a earnings
    
    # ==================== TENDENCIAS (ÚLTIMOS 30 DÍAS) ====================
    
    fecha_hace_30_dias = timezone.now() - timedelta(days=30)
    fecha_hace_60_dias = timezone.now() - timedelta(days=60)
    
    # Ingresos últimos 30 días
    earnings_last_30 = Pedido.objects.filter(
        pagado=True,
        created_at__gte=fecha_hace_30_dias
    ).aggregate(total=Sum('total'))['total'] or Decimal('0.00')
    
    # Ingresos 30 días anteriores (para calcular tendencia)
    earnings_prev_30 = Pedido.objects.filter(
        pagado=True,
        created_at__gte=fecha_hace_60_dias,
        created_at__lt=fecha_hace_30_dias
    ).aggregate(total=Sum('total'))['total'] or Decimal('0.00')
    
    # Calcular porcentaje de cambio en earnings
    if earnings_prev_30 > 0:
        earnings_change = ((earnings_last_30 - earnings_prev_30) / earnings_prev_30) * 100
    else:
        earnings_change = 100 if earnings_last_30 > 0 else 0
    
    # Pedidos últimos 30 días
    orders_last_30 = Pedido.objects.filter(created_at__gte=fecha_hace_30_dias).count()
    orders_prev_30 = Pedido.objects.filter(
        created_at__gte=fecha_hace_60_dias,
        created_at__lt=fecha_hace_30_dias
    ).count()
    
    if orders_prev_30 > 0:
        orders_change = ((orders_last_30 - orders_prev_30) / orders_prev_30) * 100
    else:
        orders_change = 100 if orders_last_30 > 0 else 0
    
    # Clientes nuevos últimos 30 días
    customers_last_30 = Usuario.objects.filter(
        rol='cliente',
        fecha_registro__gte=fecha_hace_30_dias
    ).count()
    customers_prev_30 = Usuario.objects.filter(
        rol='cliente',
        fecha_registro__gte=fecha_hace_60_dias,
        fecha_registro__lt=fecha_hace_30_dias
    ).count()
    
    if customers_prev_30 > 0:
        customers_change = ((customers_last_30 - customers_prev_30) / customers_prev_30) * 100
    else:
        customers_change = 100 if customers_last_30 > 0 else 0
    ...
    ).order_by('-total_vendidos')[:6]
    
    # ==================== PEDIDOS RECIENTES ====================
    
    recent_orders = Pedido.objects.select_related('usuario').prefetch_related(
        'items', 'items__producto'
    ).order_by('-created_at')[:5]
    
    # ==================== DATOS PARA GRÁFICAS ====================
    
    # Datos de ingresos por semana (últimas 8 semanas)
    fecha_hace_8_semanas = timezone.now() - timedelta(weeks=8)
    weekly_earnings = Pedido.objects.filter(
        pagado=True,
        created_at__gte=fecha_hace_8_semanas
    ).annotate(
    ...
    # Datos de pedidos por semana
    weekly_orders = Pedido.objects.filter(
        created_at__gte=fecha_hace_8_semanas
    ).annotate(
        week=TruncWeek('created_at')
    ).values('week').annotate(
    ...
    context = {
        ...
        'balance_change': round(earnings_change, 2),  # Mismo que earnings por ahora
        ...
        'weekly_earnings_data': list(weekly_earnings),
        'weekly_orders_data': list(weekly_orders),
    }
