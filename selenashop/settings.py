...
if PRODUCTION:
    ...
    # Preferimos variables de entorno del hosting (cPanel / Render / etc).
    # Si no se proveen, usamos defaults de Resend.
    ...
    # Resend usa API Key como "password" en SMTP.
    EMAIL_HOST_PASSWORD = (
        os.environ.get('EMAIL_HOST_PASSWORD', '').strip()
        or os.environ.get('RESEND_API_KEY', '').strip()
    )
    DEFAULT_FROM_EMAIL = os.environ.get('DEFAULT_FROM_EMAIL', '').strip() or 'Selena Shop <onboarding@resend.dev>'
